...
export class GameScene extends Phaser.Scene {
  static readonly PLACED_SPRITES_STORAGE_KEY = 'condamine-wispguard-blocking-sprites-v1';
  #levelData!: LevelData;
  ...
  #grenadeLauncherUntil = 0;
  #lastGrenadeShotAt = 0;
  #mageTextureKeys: string[] = [];
  #mageAnimationKey = '';
  #mageUsesDirectionalAttackSheet = false;
  #mageOverlay?: Phaser.GameObjects.Sprite;
  #mageFollowCallback?: () => void;
  ...
  public applyEducationalBonus(bonus: string): void {
    if (!this.#player) return;
    if (bonus === 'invincible') {
      this.#player.enableChillInvincibility();
      this.#player.restoreFullHealth();
      EVENT_BUS.emit(CUSTOM_EVENTS.SHOW_DIALOG, 'MODE INVINCIBLE ACTIF EN PERMANENCE !');
    }
    if (bonus === 'hearts') {
      this.#player.restoreFullHealth();
      EVENT_BUS.emit(CUSTOM_EVENTS.SHOW_DIALOG, 'Tes coeurs sont recharges !');
    }
    if (bonus === 'shield') {
      this.#player.grantTemporaryShield(15000);
      EVENT_BUS.emit(CUSTOM_EVENTS.SHOW_DIALOG, 'Bouclier actif pendant 15 secondes !');
    }
    if (bonus === 'super-weapon') {
      this.#player.grantSuperWeapon(20000);
      this.#activateGrenadeLauncher(20000);
      EVENT_BUS.emit(CUSTOM_EVENTS.SHOW_DIALOG, 'LANCE-GRENADES ACTIF : ESPACE pour tirer !');
    }
    if (bonus === 'mage-form') {
      this.#activateMageForm();
    }
  }

  public beginBlockingSpritePlacement(dataUrl: string): void {
    if (!dataUrl.startsWith('data:image/')) return;
    const textureKey = `condamine-import-${Date.now()}-${Math.random().toString(36).slice(2)}`;
    this.#addBase64Texture(textureKey, dataUrl, () => {
      const preview = this.add.image(this.#player.x, this.#player.y, textureKey)
        .setDisplaySize(24, 24)
        .setAlpha(0.7)
        .setDepth(60);
      const followPointer = (pointer: Phaser.Input.Pointer): void => {
        preview.setPosition(Math.round(pointer.worldX), Math.round(pointer.worldY));
      };
      const placeSprite = (pointer: Phaser.Input.Pointer): void => {
        this.input.off(Phaser.Input.Events.POINTER_MOVE, followPointer);
        this.input.off(Phaser.Input.Events.POINTER_DOWN, placeSprite);
        preview.destroy();
        const placement = {
          id: textureKey,
        ...
        this.#createBlockingSprite(placement);
        this.#saveBlockingSpritePlacement(placement);
        window.parent.postMessage({ source: 'condamine-game', type: 'sprite-placed' }, '*');
      };
      this.input.on(Phaser.Input.Events.POINTER_MOVE, followPointer);
      this.input.once(Phaser.Input.Events.POINTER_DOWN, placeSprite);
      window.parent.postMessage({ source: 'condamine-game', type: 'sprite-ready' }, '*');
    });
  }

  public beginAnimatedSpritePlacement(frames: string[]): void {
    if (frames.length < 4 || frames.some((frame) => !frame.startsWith('data:image/'))) return;
    const importId = `condamine-sheet-${Date.now()}-${Math.random().toString(36).slice(2)}`;
    const textureKeys = frames.map((_, index) => `${importId}-${index}`);
    let loadedFrames = 0;
    const frameReady = (): void => {
      loadedFrames += 1;
      if (loadedFrames !== frames.length) return;
      const animationKey = `${importId}-walk`;
      this.anims.create({
        key: animationKey,
        frames: [1, 2, 3].map((index) => ({ key: textureKeys[index] })),
        frameRate: 6,
        repeat: -1,
      });
      this.#mageTextureKeys = textureKeys;
      this.#mageAnimationKey = animationKey;
      this.#mageUsesDirectionalAttackSheet = textureKeys.length === 12;
      const preview = this.add.sprite(this.#player.x, this.#player.y, textureKeys[0])
      ...
      const placeCharacter = (pointer: Phaser.Input.Pointer): void => {
        this.input.off(Phaser.Input.Events.POINTER_MOVE, followPointer);
        this.input.off(Phaser.Input.Events.POINTER_DOWN, placeCharacter);
        preview.destroy();
        ...
        body.setSize(19, 22, true).setOffset(7, 11).setAllowGravity(false).setImmovable(true);
        this.#blockingGroup.add(character);
        window.parent.postMessage({ source: 'condamine-game', type: 'sprite-placed' }, '*');
      };
      this.input.on(Phaser.Input.Events.POINTER_MOVE, followPointer);
      this.input.once(Phaser.Input.Events.POINTER_DOWN, placeCharacter);
      window.parent.postMessage({ source: 'condamine-game', type: 'sprite-ready' }, '*');
    };
    frames.forEach((frame, index) => this.#addBase64Texture(textureKeys[index], frame, frameReady));
  }

  #activateMageForm(): void {
    if (this.#mageTextureKeys.length < 10 || this.#mageAnimationKey === '') {
      EVENT_BUS.emit(CUSTOM_EVENTS.SHOW_DIALOG, 'IMPORTE D’ABORD LA FICHE ANIMÉE DU MAGE EN HAUT DU JEU.');
      return;
    }
    if (this.#mageFollowCallback !== undefined) {
      this.events.off(Phaser.Scenes.Events.UPDATE, this.#mageFollowCallback);
    }
    this.#mageOverlay?.destroy();
    this.#player.setAlpha(0.01);
    const mage = this.add.sprite(this.#player.x, this.#player.y, this.#mageTextureKeys[0])
      .setDisplaySize(38, 38)
      .setDepth(this.#player.depth + 3);
    this.#mageOverlay = mage;
    let walking = false;
    const followMage = (): void => {
      if (!this.#player.active || !mage.active) return;
      mage.setPosition(this.#player.x, this.#player.y - 6);
      const body = this.#player.body as Phaser.Physics.Arcade.Body;
      const moving = Math.abs(body.velocity.x) > 1 || Math.abs(body.velocity.y) > 1;
      const direction = this.#player.direction;
      if (!this.#mageUsesDirectionalAttackSheet && moving && (direction === DIRECTION.LEFT || direction === DIRECTION.RIGHT)) {
        if (!walking) mage.play(this.#mageAnimationKey);
        walking = true;
        mage.setFlipX(direction === DIRECTION.LEFT);
        return;
      }
      if (walking) mage.stop();
      walking = false;
      mage.setFlipX(false);
      if (this.#mageUsesDirectionalAttackSheet) {
        const directionalFrame = {
          [DIRECTION.DOWN]: 0,
          [DIRECTION.UP]: 3,
          [DIRECTION.RIGHT]: 6,
          [DIRECTION.LEFT]: 9,
        } as const;
        mage.setTexture(this.#mageTextureKeys[directionalFrame[direction]]);
        return;
      }
      if (direction === DIRECTION.UP) mage.setTexture(this.#mageTextureKeys[7]);
      else if (direction === DIRECTION.LEFT) mage.setTexture(this.#mageTextureKeys[9]);
      else mage.setTexture(this.#mageTextureKeys[0]);
    };
    this.#mageFollowCallback = followMage;
    this.events.on(Phaser.Scenes.Events.UPDATE, followMage);
    this.events.once(Phaser.Scenes.Events.SHUTDOWN, () => {
      this.events.off(Phaser.Scenes.Events.UPDATE, followMage);
    });
    EVENT_BUS.emit(CUSTOM_EVENTS.SHOW_DIALOG, 'TRANSFORMATION EN MAGE ACTIVÉE !');
  }

  #addBase64Texture(textureKey: string, dataUrl: string, onReady: () => void): void {
    if (this.textures.exists(textureKey)) {
      onReady();
      return;
    }
    const eventName = `${Phaser.Textures.Events.ADD_KEY}${textureKey}`;
    const handleError = (failedKey: string): void => {
      if (failedKey !== textureKey) return;
      this.textures.off(Phaser.Textures.Events.ERROR, handleError);
      window.parent.postMessage({ source: 'condamine-game', type: 'sprite-error' }, '*');
    };
    const handleReady = (): void => {
      this.textures.off(Phaser.Textures.Events.ERROR, handleError);
      onReady();
    };
    this.textures.once(eventName, handleReady);
    this.textures.on(Phaser.Textures.Events.ERROR, handleError);
    this.textures.addBase64(textureKey, dataUrl);
  }

  #createBlockingSprite(placement: {
    id: string;
    level: string;
    roomId: number;
    x: number;
    ...
    const create = (): void => {
      const sprite = this.physics.add.image(placement.x, placement.y, placement.id)
        .setDisplaySize(24, 24)
        .setDepth(4)
        .setImmovable(true);
      const body = sprite.body as Phaser.Physics.Arcade.Body;
      body.setSize(22, 22, true).setAllowGravity(false).setImmovable(true);
      this.#blockingGroup.add(sprite);
    };
    if (this.textures.exists(placement.id)) {
      create();
      return;
    }
    this.#addBase64Texture(placement.id, placement.dataUrl, create);
  }

  #saveBlockingSpritePlacement(placement: {
    id: string;
    level: string;
    roomId: number;
    x: number;
    ...
    try {
      const raw = localStorage.getItem(GameScene.PLACED_SPRITES_STORAGE_KEY);
      const placements = raw === null ? [] : JSON.parse(raw);
      placements.push(placement);
      localStorage.setItem(GameScene.PLACED_SPRITES_STORAGE_KEY, JSON.stringify(placements));
    } catch (error) {
      console.warn('Impossible de mémoriser le sprite importé.', error);
    }
  }

  #restoreBlockingSprites(): void {
    try {
      const raw = localStorage.getItem(GameScene.PLACED_SPRITES_STORAGE_KEY);
      if (raw === null) return;
      const placements = JSON.parse(raw) as Array<{
        id: string;
        level: string;
        roomId: number;
        x: number;
      ...
      placements
        .filter((placement) => placement.level === this.#levelData.level)
        .forEach((placement) => this.#createBlockingSprite(placement));
    } catch (error) {
      console.warn('Impossible de restaurer les sprites importés.', error);
    }
  }

  #activateGrenadeLauncher(duration: number): void {
    this.#grenadeLauncherUntil = this.time.now + duration;
    this.time.addEvent({
      delay: 90,
      repeat: Math.ceil(duration / 90),
      callback: () => {
        if (
          this.time.now > this.#grenadeLauncherUntil ||
          !this.#grenadeKey?.isDown ||
          this.time.now - this.#lastGrenadeShotAt < 240
        ) return;
        this.#lastGrenadeShotAt = this.time.now;
        this.#fireGrenade();
      },
    });
  }
  ...
  #fireGrenade(): void {
    const vectors = {
      UP: { x: 0, y: -230, muzzleX: 0, muzzleY: -27 },
      DOWN: { x: 0, y: 230, muzzleX: 0, muzzleY: 27 },
      LEFT: { x: -230, y: 0, muzzleX: -27, muzzleY: 0 },
      RIGHT: { x: 230, y: 0, muzzleX: 27, muzzleY: 0 },
    } as const;
    const velocity = vectors[this.#player.direction];
    const grenade = this.physics.add.image(
      this.#player.x + velocity.muzzleX,
      this.#player.y + velocity.muzzleY,
      'condamine-grenade',
    )
      .setDisplaySize(11, 11)
      .setDepth(30);
    const body = grenade.body as Phaser.Physics.Arcade.Body;
    body.setCircle(5).setVelocity(velocity.x, velocity.y);
    grenade.setAngularVelocity(720);
    grenade.setData('exploded', false);
    ...
    const explode = (): void => {
      if (!grenade.active || grenade.getData('exploded')) return;
      grenade.setData('exploded', true);
      const x = grenade.x;
      const y = grenade.y;
      grenade.destroy();
      const blast = this.add.image(x, y, 'condamine-grenade-explosion')
      ...
      const shockwave = this.add.circle(x, y, 8, 0xffffff, 0)
        .setStrokeStyle(3, 0xffd34d, 0.95)
        .setDepth(39);
      this.tweens.add({
        targets: shockwave,
        scale: 6,
      ...
      const enemies = this.#objectsByRoomId[this.#currentRoomId]?.enemyGroup;
      enemies?.getChildren().forEach((child) => {
        const enemy = child as CharacterGameObject;
        if (!enemy.active) return;
        if (Phaser.Math.Distance.Between(x, y, enemy.x, enemy.y) <= 48) {
          enemy.hit(this.#player.direction, 4);
        }
      });
    };

    this.physics.add.collider(grenade, this.#collisionLayer, explode);
    const enemies = this.#objectsByRoomId[this.#currentRoomId]?.enemyGroup;
    if (enemies) this.physics.add.overlap(grenade, enemies, explode);
    this.time.delayedCall(850, explode);
  }
  ...
  public create(): void {
    ...
    this.input.keyboard.on('keydown-SPACE', requestBonus);
    this.events.once(Phaser.Scenes.Events.SHUTDOWN, () => {
      this.input.keyboard?.off('keydown-SPACE', requestBonus);
    });
    ...
    const handleParentMessage = (event: MessageEvent): void => {
      ...
      if (event.data.type === 'quiz-close') {
        this.scene.resume();
        return;
      }
      if (event.data.type === 'place-blocking-sprite') {
        this.beginBlockingSpritePlacement(String(event.data.dataUrl || ''));
        return;
      }
      if (event.data.type === 'place-animated-sprite') {
        this.beginAnimatedSpritePlacement(Array.isArray(event.data.frames) ? event.data.frames : []);
        return;
      }
      if (event.data.type !== 'grant-bonus') return;
      this.applyEducationalBonus(String(event.data.bonus || ''));
    };
    window.addEventListener('message', handleParentMessage);
    this.events.once(Phaser.Scenes.Events.SHUTDOWN, () => window.removeEventListener('message', handleParentMessage));
  ...
  #createEnemies(map: Phaser.Tilemaps.Tilemap, layerName: string, roomId: number): void {
    ...
    // Ajoute un adversaire supplémentaire dans les salles de combat normales.
    // Le léger décalage évite de superposer les deux ennemis au chargement.
    const reinforcement = validTiledObjects.find((enemy) => enemy.type === 1 || enemy.type === 2);
    if (reinforcement !== undefined) {
      const position = { x: reinforcement.x + 18, y: reinforcement.y + 18 };
      const extraEnemy = reinforcement.type === 1
        ? new Wisp({ scene: this, position })
        : new Spider({ scene: this, position });
      this.#objectsByRoomId[roomId].enemyGroup.add(extraEnemy);
    }
  }
