...
export class TitleScene extends BaseScene {
    ...
    this.cameras.main.once(Phaser.Cameras.Scene2D.Events.FADE_OUT_COMPLETE, () => {
      ...
      if (this.#selectedMenuOption === MAIN_MENU_OPTIONS.NEW_GAME) {
        try {
          ...
          console.warn('[TitleScene] nouvelle partie lancée sans sauvegarde locale.', error);
        }
      }
  ...
  update() {
    ...
    const wasValidationKeyPressed =
      this._controls.wasSpaceKeyPressed() || this._controls.wasEnterKeyPressed();
    if (wasValidationKeyPressed) {
